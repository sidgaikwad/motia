...
<strong>Where to place step files</strong>

- Place all step files inside the top-level `steps/` directory of your project.
- You can organize steps in nested subfolders within `steps/` (e.g., `steps/payments/webhook.step.ts`, `steps/auth/login.step.ts`).
- Files outside `steps/` (like `src/steps/...`) are not discovered yet.
</Callout>
